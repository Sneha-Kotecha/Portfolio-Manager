"""
TREND SURFER - OANDA ENHANCED VERSION
====================================

Professional FX backtesting system with OANDA data integration.
Enhanced pin bar detection with comprehensive trade management.
...
class OandaAPI:
    ...
    def __init__(self):
        self.api_key = st.secrets["oanda"]["api_key"]
        self.account_id = st.secrets["oanda"]["account_id"]
    ...
    def fetch_candles(self, instrument: str, granularity: str, start_time: str, end_time: str) -> pd.DataFrame:
        """Fetch candlestick data from OANDA"""
        try:
            oanda_instrument = self.convert_symbol_to_oanda(instrument)
            oanda_granularity = self.granularity_map.get(granularity, 'H1')
...
def is_valid_trading_time(utc_dt):
    """Check if time is within valid trading hours (6:00-15:00 BST)"""
...
class DataFetcher:
    ...
    def fetch_multi_timeframe_data(symbol: str, start_date: datetime, end_date: datetime) -> Dict[str, pd.DataFrame]:
        """Enhanced multi-timeframe data fetching"""
        fetcher = DataFetcher()
...
class TrendSurferBacktester:
    ...
    def _generate_trades_with_constraints(self, pin_bars: List[PinBarResult], data: Dict[str, pd.DataFrame],
        ...
        """Generate trades with time constraints and one-per-pair logic"""
        ...
        with col_btn1:
            if st.button("🚀 Run Backtest", type="primary", use_container_width=True):
                ...
                with st.spinner(f"🔄 Running {config['duration']} backtest..."):
                    ...
                    status_text.text("🔍 Detecting pin bars...")
                    ...
                    if results.trades or results.statistics:
                        ...
                        st.success(f"🎉 {config['duration']} backtest completed!")
                        ...
                        if results.statistics.get('total_trades', 0) > 0:
                            ...
                            st.warning("⚠️ No triggered trades found. Try different parameters.")
                    else:
        ...
        if not data.empty:
            ...
            if pin_bars:
                ...
                if valid_time_pins:
                    ...
                    st.info("ℹ️ No valid pin bars detected during trading hours (6:00-15:00 BST)")
            else:
...
def detect_recent_pin_bars(data: pd.DataFrame, config: Dict) -> List:
    """Pin bar detection for live analysis returning compatible format"""
    pin_bars = []
