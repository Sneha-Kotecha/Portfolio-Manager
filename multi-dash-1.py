...
class MultiAssetOptionsStrategist:
    ...
    def calculate_bear_put_spread_accurate(self, puts_df: pd.DataFrame, current_price: float,
                                          capital: float, asset_class: str, max_risk_amount: float = None) -> Dict:
        """Accurate bear put spread calculations with user capital"""
        
        share_equivalent = 100 if asset_class != 'FOREX' else 10000
        
        # Define spread: Buy higher strike put, sell lower strike put
        buy_strike = current_price * 0.98   # 2% OTM (higher strike)
        sell_strike = current_price * 0.90  # 10% OTM (lower strike)
        
        buy_put = self._find_closest_strike(puts_df, buy_strike)
        sell_put = self._find_closest_strike(puts_df, sell_strike)
        
        if not buy_put or not sell_put:
            return {'error': 'Unable to find suitable put options for spread'}
        
        buy_premium = buy_put['lastPrice']
        sell_premium = sell_put['lastPrice']
        net_debit = buy_premium - sell_premium
        
        if net_debit <= 0:
            return {'error': 'Invalid spread pricing - no debit'}
        ...
        max_profit = max_contracts * (spread_width - net_debit) * share_equivalent
        
        # Breakeven
        breakeven = buy_put['strike'] - net_debit
        
        profit_probability = self._estimate_probability_below_breakeven(current_price, breakeven, 35)
        ...
        return {
            ...
            'probability_profit': profit_probability,
            'confidence_level': self._calculate_directional_confidence(asset_class, profit_probability, 'bearish')
        }
    
    def calculate_bear_call_spread_accurate(self, calls_df: pd.DataFrame, current_price: float,
                                           capital: float, asset_class: str, max_risk_amount: float = None) -> Dict:
        """Accurate bear call spread calculations with user capital"""
        
        share_equivalent = 100 if asset_class != 'FOREX' else 10000
        
        # Define spread: Sell lower strike call, buy higher strike call
        sell_strike = current_price * 1.02  # 2% OTM (sell)
        buy_strike = current_price * 1.10   # 10% OTM (buy)
        
        sell_call = self._find_closest_strike(calls_df, sell_strike)
        buy_call = self._find_closest_strike(calls_df, buy_strike)
        
        if not sell_call or not buy_call:
            return {'error': 'Unable to find suitable call options for spread'}
        
        sell_premium = sell_call['lastPrice']
        buy_premium = buy_call['lastPrice']
        net_credit = sell_premium - buy_premium
        
        if net_credit <= 0:
            return {'error': 'Invalid spread pricing - no credit'}
        
        # Margin requirement
        spread_width = buy_call['strike'] - sell_call['strike']
        margin_per_contract = (spread_width - net_credit) * share_equivalent
        max_contracts = int(capital * 0.5 / margin_per_contract)
        
        if max_contracts == 0:
            return {'error': 'Insufficient capital for margin requirements'}
        
        total_credit = max_contracts * net_credit * share_equivalent
        total_margin = max_contracts * margin_per_contract
        max_profit = total_credit
        ...
        return {
            ...
            'max_profit': max_profit,
            'max_loss': max_loss,
            'max_profit_pct': (max_profit / total_margin) * 100,
            'breakeven': breakeven,
            'breakeven_move_pct': ((breakeven - current_price) / current_price) * 100,
            'probability_profit': profit_probability,
            'confidence_level': self._calculate_directional_confidence(asset_class, profit_probability, 'bearish')
    ...
    def calculate_protective_put_accurate(self, puts_df: pd.DataFrame, current_price: float,
                                         capital: float, asset_class: str, max_risk_amount: float = None) -> Dict:
        """Accurate protective put calculations with user capital"""
        
        share_equivalent = 100 if asset_class != 'FOREX' else 10000
        
        # Find protective put (5-10% OTM)
        put_strike = current_price * 0.95  # 5% OTM for protection
        
        protective_put = self._find_closest_strike(puts_df, put_strike)
        
        if not protective_put:
            return {'error': 'Unable to find suitable protective put'}
        
        put_premium = protective_put['lastPrice']
        
        # Calculate position size (need to buy stock + put)
        stock_cost_per_contract = current_price * share_equivalent
        put_cost_per_contract = put_premium * share_equivalent
        total_cost_per_contract = stock_cost_per_contract + put_cost_per_contract
        
        max_contracts = int(capital / total_cost_per_contract)
        
        if max_contracts == 0:
            return {'error': f'Insufficient capital. Need ${total_cost_per_contract:,.2f} for 1 contract'}
        
        total_investment = max_contracts * total_cost_per_contract
        stock_investment = max_contracts * stock_cost_per_contract
        put_investment = max_contracts * put_cost_per_contract
        
        # Protection calculations
        protected_value = max_contracts * protective_put['strike'] * share_equivalent
        max_loss = total_investment - protected_value  # Loss if stock goes to strike
        
        # Unlimited upside potential (minus put premium)
        # Estimate at 20% stock appreciation
        estimated_stock_gain = stock_investment * 0.20
        estimated_profit = estimated_stock_gain - put_investment  # Net of put cost
        
        breakeven = current_price + put_premium  # Stock needs to rise by put premium
        
        profit_probability = self._estimate_probability_above_breakeven(current_price, breakeven, 90)
        ...
        return {
            ...
            'breakeven': breakeven,
            'breakeven_move_pct': ((breakeven - current_price) / current_price) * 100,
            'protection_level_pct': ((current_price - protective_put['strike']) / current_price) * 100,
            'probability_profit': profit_probability,
            'confidence_level': self._calculate_protection_confidence(asset_class, profit_probability)
        }
    
    def calculate_collar_accurate(self, calls_df: pd.DataFrame, puts_df: pd.DataFrame,
                                 current_price: float, capital: float, asset_class: str, max_risk_amount: float = None) -> Dict:
        """Accurate collar calculations with user capital"""
        
        share_equivalent = 100 if asset_class != 'FOREX' else 10000
        
        # Find options: Buy protective put (5% OTM), Sell covered call (5% OTM)
        put_strike = current_price * 0.95  # Protective put
        call_strike = current_price * 1.05  # Covered call
        
        protective_put = self._find_closest_strike(puts_df, put_strike)
        covered_call = self._find_closest_strike(calls_df, call_strike)
        
        if not protective_put or not covered_call:
            return {'error': 'Unable to find suitable options for collar'}
        
        put_premium = protective_put['lastPrice']
        call_premium = covered_call['lastPrice']
        net_option_cost = put_premium - call_premium  # Can be negative (credit) or positive (debit)
        
        # Calculate position size
        stock_cost_per_contract = current_price * share_equivalent
        option_cost_per_contract = net_option_cost * share_equivalent if net_option_cost > 0 else 0
        total_cost_per_contract = stock_cost_per_contract + option_cost_per_contract
        ...
        stock_investment = max_contracts * stock_cost_per_contract
        net_option_cost_total = max_contracts * net_option_cost * share_equivalent
        total_investment = stock_investment + max(0, net_option_cost_total)
        
        # Collar profit/loss calculations
        if current_price <= protective_put['strike']:  # Stock drops to put strike
            collar_value = max_contracts * protective_put['strike'] * share_equivalent
            pnl_at_put = collar_value - total_investment
        else:
            pnl_at_put = 0
        
        if current_price >= covered_call['strike']:  # Stock rises to call strike
            collar_value = max_contracts * covered_call['strike'] * share_equivalent
            pnl_at_call = collar_value - stock_investment + net_option_cost_total
        else:
            pnl_at_call = 0
        
        max_profit = max_contracts * (covered_call['strike'] - current_price) * share_equivalent + net_option_cost_total
        max_loss = total_investment - max_contracts * protective_put['strike'] * share_equivalent
        
        profit_probability = 0.65  # Collars typically have good success rates
        ...
        return {
            ...
            'stock_price': current_price,
            'put_strike': protective_put['strike'],
            'call_strike': covered_call['strike'],
            'put_premium': put_premium,
            'call_premium': call_premium,
            ...
            'max_profit': max_profit,
            'max_loss': max_loss,
            'profit_range': covered_call['strike'] - protective_put['strike'],
            'profit_range_pct': ((covered_call['strike'] - protective_put['strike']) / current_price) * 100,
            'protection_level_pct': ((current_price - protective_put['strike']) / current_price) * 100,
            'probability_profit': profit_probability,
            'confidence_level': self._calculate_collar_confidence(asset_class, profit_probability)
        }
    ...
    def _estimate_probability_below_breakeven(self, current_price: float, breakeven: float, days: int) -> float:
        """Estimate probability of price being below breakeven (for bearish strategies)"""
    ...
    def _calculate_straddle_confidence(self, asset_class: str, profit_probability: float, is_short: bool = False) -> str:
        ...
        if is_short:
            ...
            # Long straddles need big moves
            if profit_probability > 0.4:
                return "HIGH"
            elif profit_probability > 0.25:
                return "MODERATE"
            else:
                return "LOW"
    ...
    def _calculate_strangle_confidence(self, asset_class: str, profit_probability: float, is_short: bool = False) -> str:
        ...
        if is_short:
            ...
            if profit_probability > 0.35:
                return "HIGH"
            elif profit_probability > 0.2:
                return "MODERATE"
            else:
                return "LOW"
    ...
    def _calculate_protection_confidence(self, asset_class: str, profit_probability: float) -> str:
        """Calculate confidence level for protective strategies"""
        
        # Protection strategies generally have higher confidence
    ...
    def _calculate_collar_confidence(self, asset_class: str, profit_probability: float) -> str:
        """Calculate confidence level for collar strategies"""
        
        # Collars are conservative strategies with decent success rates
        if profit_probability > 0.7:
            return "HIGH"
        elif profit_probability > 0.55:
            return "MODERATE"
    ...
    def analyze_all_strategies(self, calls_df: pd.DataFrame, puts_df: pd.DataFrame,
        ...
        strategies_to_analyze = [
            ...
            ('BEAR_PUT_SPREAD', self.calculate_bear_put_spread_accurate),
            ('BEAR_CALL_SPREAD', self.calculate_bear_call_spread_accurate),
            ...
            ('PROTECTIVE_PUT', self.calculate_protective_put_accurate),
            ('COLLAR', self.calculate_collar_accurate)
        ]
        ...
        for strategy_name, strategy_func in strategies_to_analyze:
            try:
                if strategy_name in ['COVERED_CALL']:
                    result = strategy_func(calls_df, current_price, capital, asset_class, max_risk_amount)
                elif strategy_name in ['CASH_SECURED_PUT', 'BEAR_PUT_SPREAD']:
                    result = strategy_func(puts_df, current_price, capital, asset_class, max_risk_amount)
                elif strategy_name in ['BULL_CALL_SPREAD', 'BEAR_CALL_SPREAD']:
                    result = strategy_func(calls_df, current_price, capital, asset_class, max_risk_amount)
                elif strategy_name in ['PROTECTIVE_PUT']:
                    result = strategy_func(puts_df, current_price, capital, asset_class, max_risk_amount)
    ...
    def _get_market_alignment_bonus(self, strategy_name: str, trend: str, volatility: str) -> float:
        ...
        # Trend alignment
        bullish_strategies = ['COVERED_CALL', 'BULL_CALL_SPREAD', 'CASH_SECURED_PUT', 'PROTECTIVE_PUT']
        bearish_strategies = ['BEAR_PUT_SPREAD', 'BEAR_CALL_SPREAD']
        neutral_strategies = ['IRON_CONDOR', 'SHORT_STRADDLE', 'SHORT_STRANGLE', 'COLLAR']
        volatile_strategies = ['LONG_STRADDLE', 'LONG_STRANGLE']
        
        if strategy_name in bullish_strategies and 'BULLISH' in trend:
            bonus += 8
        elif strategy_name in bearish_strategies and 'BEARISH' in trend:
            bonus += 8
...
def display_enhanced_strategy_results(results: Dict, asset_class: str, expiration_date: str, max_risk_amount: float):
    """Display strategy results with detailed trade instructions - FIXED"""
...
def display_single_trade_instruction(rec: Dict, asset_class: str, strategy: str, expiration: str, max_risk_amount: float):
    """Display detailed trade instructions for a single recommendation - Updated for all strategies"""
    ...
    with col1:
        ...
        if strategy == 'COVERED_CALL':
            ...
            **Expiration:** {expiration}
            ...
            st.markdown(f"""
            **Step 1:** Buy Long Put
            - **Buy:** {rec['contracts']} puts at {price_format.format(rec['buy_strike'])}
            - **Premium Paid:** ${rec['contracts'] * rec['buy_premium'] * (100 if asset_class != 'FOREX' else 10000):,.0f}
            
            **Step 2:** Sell Short Put
            - **Sell:** {rec['contracts']} puts at {price_format.format(rec['sell_strike'])}
            - **Premium Received:** ${rec['contracts'] * rec['sell_premium'] * (100 if asset_class != 'FOREX' else 10000):,.0f}
            ...
            **Expiration:** {expiration}
            ...
            st.markdown(f"""
            **Step 1:** Sell Short Call
            - **Sell:** {rec['contracts']} calls at {price_format.format(rec['sell_strike'])}
            - **Premium Received:** ${rec['contracts'] * rec['sell_premium'] * (100 if asset_class != 'FOREX' else 10000):,.0f}
            
            **Step 2:** Buy Long Call
            - **Buy:** {rec['contracts']} calls at {price_format.format(rec['buy_strike'])}
            - **Premium Paid:** ${rec['contracts'] * rec['buy_premium'] * (100 if asset_class != 'FOREX' else 10000):,.0f}
            ...
            **Step 1:** Buy ATM Call
            ...
            - **Premium Paid:** ${rec['contracts'] * rec['call_premium'] * (100 if asset_class != 'FOREX' else 10000):,.0f}
            
            **Step 2:** Buy ATM Put
            - **Buy:** {rec['contracts']} puts at {price_format.format(rec['strike'])}
            - **Premium Paid:** ${rec['contracts'] * rec['put_premium'] * (100 if asset_class != 'FOREX' else 10000):,.0f}
            ...
            **Step 1:** Sell ATM Call
            ...
            - **Premium Received:** ${rec['contracts'] * rec['call_premium'] * (100 if asset_class != 'FOREX' else 10000):,.0f}
            
            **Step 2:** Sell ATM Put
            - **Sell:** {rec['contracts']} puts at {price_format.format(rec['strike'])}
            - **Premium Received:** ${rec['contracts'] * rec['put_premium'] * (100 if asset_class != 'FOREX' else 10000):,.0f}
            ...
            **Expiration:** {expiration}
            ...
            st.markdown(f"""
            **Step 1:** Buy Stock Position
            - **Buy:** {rec['contracts'] * (100 if asset_class != 'FOREX' else 10000):,} shares/units
            - **Stock Cost:** ${rec['stock_investment']:,.0f}
            
            **Step 2:** Buy Protective Put
            - **Buy:** {rec['contracts']} puts at {price_format.format(rec['put_strike'])}
            - **Put Cost:** ${rec['put_investment']:,.0f}
            
            **Total Investment:** ${rec['total_investment']:,.0f}
            **Protected Value:** ${rec['protected_value']:,.0f}
            **Expiration:** {expiration}
            """)
            ...
            **Net Option Cost:** ${rec.get('net_option_cost', 0) * rec['contracts'] * (100 if asset_class != 'FOREX' else 10000):,.0f}
            **Total Investment:** ${rec['total_investment']:,.0f}
            **Expiration:** {expiration}
            """)
    ...
    with col2:
        ...
        if strategy == 'PROTECTIVE_PUT' and 'protection_level_pct' in rec:
            st.write(f"• **Protection Level:** {rec['protection_level_pct']:.1f}% downside")
        
        if strategy in ['COLLAR'] and 'protection_level_pct' in rec:
            st.write(f"• **Protection:** {rec['protection_level_pct']:.1f}% downside")
    ...
    suitability_messages = {
        ...
        'BEAR_CALL_SPREAD': "Excellent for bearish outlook with high IV environment",
        'LONG_STRADDLE': "Best before expected volatility expansion or major events",
        'SHORT_STRADDLE': "Ideal for range-bound markets with high implied volatility",
        'LONG_STRANGLE': "Perfect for expecting big moves but cheaper than straddle",
        'SHORT_STRANGLE': "Great for stable markets with elevated implied volatility",
        'PROTECTIVE_PUT': "Insurance strategy for long stock positions in uncertain markets",
        'COLLAR': "Conservative strategy for capping upside to pay for downside protection"
    }
